...
public static class BodyTrackingSetup
{
    ...
    /// <summary>
    /// Applied only to the debug APK so it installs alongside the upstream release build instead
    /// of replacing it. Android refuses to update a package whose signature differs, and upstream's
    /// APK is signed with their keystore while this one uses Unity's auto-generated debug keystore.
    /// A separate id sidesteps that, and keeps the upstream app plus its PC Service pairing intact.
    /// </summary>
    private const string DevApplicationId = "com.xrobotoolkit.client.quest.bodydev";

    private const string DevProductName = "XRoboToolkit BodyDev";
    ...
    public static void BuildDebugApk()
    {
        ...
        // Restored in the finally below: these setters write through to ProjectSettings.asset, and
        // leaking a dev id into it would silently change what the release build produces.
        var prevId = PlayerSettings.GetApplicationIdentifier(BuildTargetGroup.Android);
        var prevName = PlayerSettings.productName;

        UnityEditor.Build.Reporting.BuildSummary summary;
        try
        {
            PlayerSettings.SetApplicationIdentifier(BuildTargetGroup.Android, DevApplicationId);
            PlayerSettings.productName = DevProductName;
            Debug.Log($"[BodyTrackingSetup] applicationIdentifier={DevApplicationId}");

            summary = BuildPipeline.BuildPlayer(new BuildPlayerOptions
            {
        ...
        finally
        {
            ...
            AssetDatabase.SaveAssets();
            Debug.Log($"[BodyTrackingSetup] applicationIdentifier restored to {prevId}");
        }
